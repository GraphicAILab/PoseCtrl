...
import numpy as np
from diffusers import DDPMScheduler, PNDMScheduler, LMSDiscreteScheduler
...
def parse_args():
    ...
    parser.add_argument("--weight_decay", type=float, default=3e-2, help="Weight decay to use.")
    ...
    for epoch in range(0, args.num_train_epochs): #default is 100
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(pose_ctrl):
                ...
                uncond_image_emdeds = torch.zeros_like(image_embeds)
                noise_pred_cond = pose_ctrl(noisy_latents, timesteps, encoder_hidden_states, batch['view_matrix'], batch['projection_matrix'], image_embeds)
                noise_pred_uncond = pose_ctrl(noisy_latents, timesteps, encoder_hidden_states, batch['view_matrix'], batch['projection_matrix'], uncond_image_emdeds)
                cfg_scale = 5.5  
